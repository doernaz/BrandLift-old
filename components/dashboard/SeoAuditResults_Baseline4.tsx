...
const getIndustryImages = (type: string = '') => {
    ...
    if (t.includes('plumb') || t.includes('hvac') || t.includes('heat') || t.includes('contractor') || t.includes('construct')) {
        return [
            "https://images.unsplash.com/photo-1581094794329-cd1096a7a2e8?auto=format&fit=crop&w=1200&q=80", // Modern Pipe/Tech (Fixed)
    ...
    const { safeResult, primaryVariant, impactMetrics, hasWebsite, generatedVariants } = React.useMemo(() => {
        try {
            ...
            const getIndustryOpportunities = (ind: string) => {
                ...
                if (i.includes('hvac') || i.includes('air') || i.includes('heating')) {
                    ops = ["Emergency AC Repair Schema", "Seasonal Tune-up Landing Pages", "Local Service Area Optimization", "Energy Efficiency Rebate Content"];
            ...
            // 3. Generate Variants (Hardcoded to 2 unique visual directions)
            const generatedVariants: SeoVariant[] = [];

            // Variant 1: Cyber-Forward Interstellar (Futuristic)
            generatedVariants.push({
                name: "Cyber-Forward Interstellar",
                description: "Futuristic, high-contrast dark mode with neon accents. Reimagining your brand for the Web3 era.",
                optimizedScore: 99,
                opportunities: finalOpportunities,
                seoPackage: {
                    jsonLd: { LocalBusiness: { name: businessName, address: selectedLead?.address } } as any,
                    metaTags: { title: `${businessName}`, description: `Best ${businessType}` },
            ...
            // Variant 2: Premium Minimalist (Clean, Image-Driven)
            generatedVariants.push({
                name: "Premium Minimalist",
                description: "Clean lines, heavy whitespace, and an image-driven layout. Communicating pure professionalism and trust.",
                optimizedScore: 97,
                opportunities: finalOpportunities,
                seoPackage: {
                    jsonLd: { LocalBusiness: { name: businessName, address: selectedLead?.address } } as any,
            ...
            // Variant 3: Whimsical & Engaging (Cartoon/Playful)
            generatedVariants.push({
                name: "Whimsical & Engaging",
                description: "A fun, approachable 'cartoon' style aesthetic. High saturation and playful elements to maximize user engagement.",
                optimizedScore: 96,
                opportunities: finalOpportunities,
                seoPackage: {
                    jsonLd: { LocalBusiness: { name: businessName, address: selectedLead?.address } } as any,
...
function generatePreviewHtml(type: string, data: any) {
    ...
    // 2. FUTURIST (Dark Mode, Cyan, Space Grotesk)
        ...
        <body class="bg-black text-white antialiased selection:bg-cyan-500 selection:text-black">
            <header class="fixed w-full z-50 backdrop-blur-md border-b border-white/10">
                <div class="max-w-7xl mx-auto px-6 py-6 flex justify-between items-center">
                    <div class="text-2xl font-bold tracking-tighter uppercase">${brandName}<span class="text-cyan-500">.AI</span></div>
                    <a href="#" class="border border-cyan-500 text-cyan-500 px-6 py-2 rounded-full hover:bg-cyan-500 hover:text-black transition-all text-sm font-bold uppercase tracking-widest">Connect</a>
                </div>
            </header>

            <main class="relative overflow-hidden">
                <!-- Background Glow -->
                <div class="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[800px] bg-cyan-500/20 rounded-full blur-[120px] -z-10 pointer-events-none"></div>

                <section class="min-h-screen flex flex-col justify-center items-center text-center px-6 pt-20">
                    <div class="fade-in">
                        <div class="inline-block px-4 py-1 border border-white/20 rounded-full text-xs font-mono text-cyan-400 mb-6 bg-white/5 backdrop-blur">NEXT_GEN PROPERTY</div>
                        <h1 class="text-6xl md:text-8xl font-black mb-8 leading-none tracking-tighter">
                            FUTURE <br/><span class="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-600">READY</span>
                        </h1>
                        <p class="text-xl text-slate-400 max-w-2xl mx-auto mb-10 font-light border-l-2 border-cyan-500 pl-6 text-left">
                            ${description} Redefining boundaries with cutting-edge infrastructure.
                        </p>
                        <div class="flex gap-6 justify-center">
                            <button class="bg-cyan-500 text-black px-10 py-4 rounded-none font-bold text-lg hover:scale-105 transition-transform uppercase tracking-widest">Initialize</button>
                        </div>
                    </div>
                </section>
                ...
                <section class="py-32 px-6 border-t border-white/10">
                    <div class="max-w-7xl mx-auto grid md:grid-cols-2 gap-20">
                        ...
                        <div class="group cursor-pointer">
                            <div class="text-xs font-mono text-cyan-500 mb-2">0${i + 1} // MODULE</div>
                            <h3 class="text-4xl font-bold mb-4 group-hover:text-cyan-400 transition-colors">${s}</h3>
                            <div class="h-0.5 w-12 bg-white/20 group-hover:w-full group-hover:bg-cyan-500 transition-all duration-500"></div>
                        </div>
                        `).join('')}
                    </div>
                </section>
    ...
    // 3. MINIMALIST (Clean, Serif, Stone)
        ...
        <body class="bg-[#fcfbf9] text-[#1c1917] antialiased">
            <header class="fixed w-full z-50 bg-[#fcfbf9]/80 backdrop-blur">
                <div class="max-w-5xl mx-auto px-6 py-8 flex justify-between items-center">
                    <div class="text-xl font-medium tracking-tight">${brandName}</div>
                    <nav class="hidden md:flex gap-12">
                        <a href="#" class="text-sm text-stone-500 hover:text-black transition-colors">Work</a>
                        <a href="#" class="text-sm text-stone-500 hover:text-black transition-colors">Studio</a>
                        <a href="#" class="text-sm text-stone-500 hover:text-black transition-colors">Contact</a>
                    </nav>
                </div>
            </header>

            <main class="pt-40 pb-20">
                <section class="max-w-5xl mx-auto px-6 fade-in">
                    <h1 class="text-5xl md:text-7xl font-light tracking-tight mb-12 leading-tight">
                        Essential.<br/>Effective.<br/><span class="text-stone-400">Timeless.</span>
                    </h1>
                    <div class="grid md:grid-cols-2 gap-12 items-start">
                        <div class="text-lg leading-relaxed text-stone-600">
                           ${description} We strip away the unnecessary to reveal the core value.
                        </div>
                        <div class="h-[600px] bg-stone-200 overflow-hidden">
                             <img src="${heroImage}" class="w-full h-full object-cover grayscale opacity-90 hover:opacity-100 hover:scale-105 transition-all duration-[1.5s]">
                        </div>
                    </div>
                </section>
            </main>
    ...
    // 4. ARTISAN (Playfair Display, Purple, Sophisticated)
        ...
        <body class="bg-[#1a0b2e] text-[#e9d5ff] antialiased selection:bg-purple-500 selection:text-white">
            <nav class="fixed w-full z-50 py-8 px-8 flex justify-between items-center mix-blend-difference">
                <div class="text-2xl font-italic">${brandName}</div>
                <div class="w-8 h-8 rounded-full border border-white flex items-center justify-center text-xs">Menu</div>
            </nav>

            <main>
                <section class="min-h-screen flex items-center px-8 lg:px-24 pt-20">
                    <div class="grid lg:grid-cols-2 gap-16 items-center w-full">
                        <div class="fade-in">
                            <div class="text-purple-400 font-sans text-xs tracking-[0.3em] uppercase mb-6">Est. 2024</div>
                            <h1 class="text-6xl lg:text-8xl mb-8 leading-[0.9]">
                                Crafted <br/><span class="italic text-purple-300">Excellence</span>
                            </h1>
                            <p class="text-xl text-purple-100/60 font-sans max-w-md mb-12 leading-relaxed">
                                ${description} Where strategy meets artistry.
                            </p>
                            <a href="#" class="inline-block border-b border-purple-400 pb-2 text-purple-400 hover:text-white hover:border-white transition-all font-sans text-sm uppercase tracking-widest">Discover More</a>
                        </div>
                        <div class="relative aspect-[3/4] lg:aspect-[4/5] fade-in" style="animation-delay: 0.3s;">
                            <div class="absolute inset-0 border border-purple-500/30 transform translate-x-4 translate-y-4"></div>
                            <img src="${heroImage}" class="w-full h-full object-cover relative z-10 opacity-80 hover:opacity-100 transition-opacity duration-700">
                        </div>
                    </div>
                </section>
            </main>
