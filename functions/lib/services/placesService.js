...
const fetchPlacesPage = async (filters, pageToken, customQuery) => {
    var _a;
    ...
    if (!response.ok) {
        ...
        throw new Error(((_a = errorData.error) === null || _a === void 0 ? void 0 : _a.message) || 'API Error');
    }
    return response.json();
};
const searchLeads = async (filters) => {
    let allRawPlaces = [];
    const scannedPlaceIds = new Set();
    // Helper to execute a scan pattern
    const executeScan = async (query) => {
        ...
        do {
            try {
                ...
                const data = await fetchPlacesPage(filters, nextPageToken, query);
                if (data.places) {
                    let newCount = 0;
    ...
    const currentNoWebsiteCount = allRawPlaces.filter((p) => !p.websiteUri).length;
    ...
    const leads = allRawPlaces.map((p) => {
        var _a;
        return ({
            id: p.id,
            name: ((_a = p.displayName) === null || _a === void 0 ? void 0 : _a.text) || 'Unknown Business',
            ...
            reviews: p.reviews ? p.reviews.map((r) => {
                var _a, _b, _c;
                return ({
                    name: ((_a = r.authorAttribution) === null || _a === void 0 ? void 0 : _a.displayName) || 'Anonymous',
                    rating: r.rating,
                    text: ((_b = r.text) === null || _b === void 0 ? void 0 : _b.text) || '',
                    relativePublishTimeDescription: r.relativePublishTimeDescription,
                    authorPhotoUri: (_c = r.authorAttribution) === null || _c === void 0 ? void 0 : _c.photoUri
                });
            }) : [],
    ...
    const enrichedLeads = await (0, enrichmentService_1.enrichLeadsWithEmail)(leadsToProcess);
    // Calculate final stats
    const finalStats = {
        scanned: allRawPlaces.length,
        noWebsite: allRawPlaces.filter((p) => !p.websiteUri).length
    };
    return {
