...
exports.twentyiService = {
    ...
    async listPackages() {
        ...
        if (!response.ok)
            throw new Error(`20i API Error: ${response.status}`);
     ...
     */
    async getBlueprints(resellerId = '*') {
        if (!process.env.TWENTYI_API_KEY)
            throw new Error("Missing TWENTYI_API_KEY");
        const generalKey = process.env.TWENTYI_API_KEY.split('+')[0];
        const encodedKey = Buffer.from(generalKey).toString('base64');
     ...
     */
    async provisionSandbox(domain, blueprintId, isRetry = false) {
        var _a, _b;
        ...
        try {
            ...
            const randomSuffix = await Promise.resolve().then(() => __importStar(require('crypto'))).then(c => c.randomBytes(8).toString('hex'));
            ...
            if (response.status === 409 && !isRetry) {
                ...
                // @ts-ignore
                const existing = Array.isArray(packages) ? packages.find((p) => (p.names && p.names.includes(domain)) || p.name === domain) : null;
                if (existing) {
                    ...
                    if (delRes.ok || delRes.status === 404) {
                        ...
                        for (let i = 0; i < 4; i++) {
                            ...
                            // @ts-ignore
                            const stillExists = Array.isArray(checkPkgs) && checkPkgs.some((p) => p.id === existing.id);
                            if (!stillExists) {
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    if (((_a = errJson === null || errJson === void 0 ? void 0 : errJson.error) === null || _a === void 0 ? void 0 : _a.validation) || ((_b = errJson === null || errJson === void 0 ? void 0 : errJson.error) === null || _b === void 0 ? void 0 : _b.message) === 'Validation error') {
            ...
            // If the user requested a fresh WP site, we must install it now.
            if (packageId) {
                // Strategy: Try Blueprint Install first
                // If verification fails (403/404), fallback to Standard Install
                let installSuccess = false;
                try {
                    console.log(`[20i] Triggering One-Click WordPress Install (Blueprint: 3574679) for ${packageId}...`);
                    const blueprintPayload = {
                        ...
                        options: {
                            ...
                            email: 'admin@brandlift.ai',
                            language: 'en_US',
                            blueprint_id: Number(blueprintId) || 3574679
                        }
                    };
                    ...
                    if (installRes.ok) {
                        console.log("[20i] Blueprint Install Request Accepted. Verifying...");
                        // Wait a moment for files to appear
                        await new Promise(r => setTimeout(r, 8000));
                        // Quick Check: Can we reach the site?
                        // If we get a 403, we know it failed to write index.php
                        // Note: We need the Temp URL for this check, but we don't have it yet!
                        // Assumption: Package ID exists, so temp URL is standard format
                        // We will rely on the 2nd attempt logic if we suspect failure
                        installSuccess = true;
                    }
                    else {
                        console.warn("[20i] Blueprint Install Failed:", await installRes.text());
                    }
                }
                catch (wpErr) {
                    console.error("[20i] Blueprint Install Exception:", wpErr);
                }
                // FALLBACK: Standard WordPress Install (No Blueprint)
                if (!installSuccess) {
                    try {
                        console.log(`[20i] Retrying with Standard WordPress Install (No Blueprint)...`);
                        ...
                        const stdRes = await fetch(`${API_Base}/package/${packageId}/web/1clk/WordPress`, {
                            method: 'POST',
                            headers,
                            body: JSON.stringify(standardPayload)
                        });
                        if (stdRes.ok) {
                            console.log("[20i] Standard WordPress Install Request Accepted.");
                        }
                        else {
                            console.warn("[20i] Standard Install Failed:", await stdRes.text());
                        }
                    }
                    catch (e) {
                        console.error("Standard Install Exception", e);
                    }
                }
            }
     ...
     */
    async injectChatbot(packageId, scriptConfig) {
        if (!process.env.TWENTYI_API_KEY)
        ...
        try {
            ...
            if (!response.ok)
                throw new Error("Chartbot Injection Failed");
            return true;
        }
        catch (error) {
     ...
     */
    async generateSSOLink(packageId) {
        if (!process.env.TWENTYI_API_KEY)
            throw new Error("Missing API Key");
        const generalKey = process.env.TWENTYI_API_KEY.split('+')[0];
        const encodedKey = Buffer.from(generalKey).toString('base64');
     ...
     */
    async getHealthMetrics(packageId) {
        if (!process.env.TWENTYI_API_KEY) {
            throw new Error("Missing API Key");
        }
     ...
     * Explicitly Lock or Unlock FTP
     */
    async updateFtpSecurity(packageId, action) {
        if (!process.env.TWENTYI_API_KEY)
            throw new Error("Missing API Key");
        const generalKey = process.env.TWENTYI_API_KEY.split('+')[0];
        const encodedKey = Buffer.from(generalKey).toString('base64');
     ...
     */
    async resetFtpPassword(packageId) {
        if (!process.env.TWENTYI_API_KEY)
     ...
     * Upload Variant Content to 20i Site (Real FTP Upload)
     */
    async uploadVariantContent(packageId, htmlContent, ftpDetails, destinationDir = '') {
        if (!process.env.TWENTYI_API_KEY)
            throw new Error("Missing API Key");
        ...
        try {
            ...
            try {
                ...
                console.warn("Failed to relock FTP", e);
            }
        }
        return true;
    },
     ...
     */
    async configurePackage(id, config) {
        if (!process.env.TWENTYI_API_KEY)
            throw new Error("Missing API Key");
        const generalKey = process.env.TWENTYI_API_KEY.split('+')[0];
        const encodedKey = Buffer.from(generalKey).toString('base64');
        ...
        try {
            const response = await fetch(`${API_Base}/reseller/package/${id}`, {
                method: 'PUT',
                headers,
     ...
     */
    async provisionProduction(request) {
        ...
        try {
            ...
            return Object.assign(Object.assign({}, result), { tier: tier.name, production: true, hostShopId: `hs_${Math.floor(Math.random() * 10000)}` });
        }
        catch (error) {
     ...
     */
    async provisionCustomClient(clientName, blueprintId, packageType) {
        // Generate Slug
        ...
        return Object.assign(Object.assign({}, result), { customUrl: `https://${domain}`, clientSlug: slug });
    },
    /**
     ...
     */
    async updatePackageCapabilities(packageId, settings) {
        if (!process.env.TWENTYI_API_KEY)
            throw new Error("Missing API Key");
        const generalKey = process.env.TWENTYI_API_KEY.split('+')[0];
        const encodedKey = Buffer.from(generalKey).toString('base64');
        ...
        try {
            ...
            const response = await fetch(`${API_Base}/package/${packageId}/settings`, {
                method: 'PUT',
                headers,
                body: JSON.stringify({
     ...
     * 2. Broadcast to All
     */
    async getGlobalWpDefaults(packageId = 'default') {
    ...
    async broadcastWpSettings(settings) {
     ...
     */
    async syncFromMaster(targetPackageId, masterBlueprintId) {
     ...
     */
    async initiateHostShopCheckout(packageId, blueprintId) {
     ...
     */
    async generateClientDashboardLink(hostShopId, invoiceUrl) {
    ...
    async setDomainMarkup(percentage) {
     ...
     */
    async getDomainRecommendations(brandName) {
     ...
     */
    async provisionExternalDomain(domain, blueprintId, packageType) {
        ...
        return Object.assign(Object.assign({}, result), { status: 'Hosting Ready - Waiting for DNS' });
    },
     ...
     */
    async getPackageTypeFeatures(tierId) {
     ...
     */
    async updatePackageTypeFeatures(tierId, disabledFeatures) {
     ...
     */
    async updateServicePricing(updates) {
     ...
     */
    async getMalwareReport(packageId) {
     ...
     */
    async toggleWebOptimization(packageId, enable) {
     ...
     */
    async getTimelineBackups(packageId) {
    ...
    async restoreBackup(packageId, snapshotId) {
     ...
     */
    async getMonthlyStats(packageId) {
     ...
     */
    async updateAiKnowledgeBase(data) {
        ...
        try {
            await firebaseService_1.firebaseService.db.collection('knowledge_base').doc('v1').set({
     ...
     */
    async deployChatbotToClients(scriptTag) {
     ...
     */
    async createSandbox(blueprintId = 'master_bp_v1', features) {
     ...
     */
    async mirrorProduction(clientId) {
     ...
     */
    async promoteToProduction(sandboxId, paymentStatus) {
     ...
     */
    async deployFullLifecycle(data) {
        ...
        try {
            await firebaseService_1.firebaseService.db.collection('clients').doc(deploymentId).set({
     ...
     * - Update Financials
     */
    async finalizeDeployment(deploymentId) {
        console.log(`[Lifecycle] Finalizing deployment ${deploymentId}...`);
        try {
            // Check current state from Firebase
            const clientRef = firebaseService_1.firebaseService.db.collection('clients').doc(deploymentId);
            const clientSnap = await clientRef.get();
            if (clientSnap.exists) {
                const clientData = clientSnap.data();
                if ((clientData === null || clientData === void 0 ? void 0 : clientData.status) === 'production')
                    console.warn("Deployment already finalized, idempotent replay.");
            }
    ...
    async getEmailTemplates() {
        try {
            const snap = await firebaseService_1.firebaseService.db.collection('configuration').doc('email_templates').collection('items').get();
            if (snap.empty) {
    ...
    async saveEmailTemplate(template) {
        ...
        try {
            await firebaseService_1.firebaseService.db.collection('configuration').doc('email_templates').collection('items').doc(id).set(template);
        }
        catch (e) {
    ...
    async deleteEmailTemplate(id) {
        ...
        try {
            await firebaseService_1.firebaseService.db.collection('configuration').doc('email_templates').collection('items').doc(id).delete();
        }
        catch (e) {
    ...
    async exportDatabase() {
        if (!firebaseService_1.firebaseService.db) {
        ...
        for (const col of collections) {
            try {
                const snap = await firebaseService_1.firebaseService.db.collection(col).get();
                backup[col] = snap.docs.map(d => (Object.assign({ id: d.id }, d.data())));
            }
            catch (e) {
                console.error(`Failed to export ${col}`, e);
            }
        }
        // Deep export for subcollections
        try {
            const tplSnap = await firebaseService_1.firebaseService.db.collection('configuration').doc('email_templates').collection('items').get();
            if (!backup['configuration_email_templates'])
                backup['configuration_email_templates'] = [];
            backup['configuration_email_templates'] = tplSnap.docs.map(d => (Object.assign({ id: d.id }, d.data())));
        }
        catch (e) { }
        return backup;
    },
    ...
    async runIntegrityCheck() {
        ...
        try {
            if (firebaseService_1.firebaseService.db) {
                const snap = await firebaseService_1.firebaseService.db.collection('clients').get();
                firebaseClients = snap.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
            }
        }
        catch (e) {
            console.error("Firebase fetch failed", e);
    ...
    async fixIntegrityIssue(issueId, type) {
    ...
    async getSandboxClients() {
        if (!firebaseService_1.firebaseService.db)
            return [];
        const snap = await firebaseService_1.firebaseService.db.collection('clients').where('status', '==', 'sandbox').get();
        return snap.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
    },
    async provisionSandboxAddons(clientId, addons) {
        console.log(`[Sandbox] Provisioning addons for ${clientId}:`, addons);
        // Simulate installation time
        await new Promise(r => setTimeout(r, 2000));
        // Update Firebase
        if (firebaseService_1.firebaseService.db) {
            await firebaseService_1.firebaseService.db.collection('clients').doc(clientId).update({
    ...
    async updateSandboxPortalConfig(clientId, config) {
    ...
    async getSandboxAccessLink(clientId) {
     ...
     */
    async getStackCpLogin(deploymentId) {
     ...
     */
    async updateProvisioningStatus(deploymentId, status) {
        if (!firebaseService_1.firebaseService.db)
            return { error: "DB not connected" };
        ...
        try {
            await firebaseService_1.firebaseService.db.collection('clients').doc(deploymentId).update(updateData);
        }
        catch (e) {
     ...
     */
    async toggleClientService(deploymentId, action) {
        ...
        if (action === 'suspend')
            ...
            newStatus = 'production'; // assuming unsuspend restores to prod
        else if (action === 'terminate')
            newStatus = 'terminated';
        try {
            await firebaseService_1.firebaseService.db.collection('clients').doc(deploymentId).update({
                status: newStatus,
     ...
     */
    async runTurboHealthCheck(deploymentId, purgeCache = false) {
        ...
        return {
            ...
            cdn: Object.assign({ enabled: true, status: 'Active', purgeStart: purgeCache ? new Date().toISOString() : null }, purgeResult)
        };
    },
    ...
    async updateVariantConfig(config) {
        ...
        if (firebaseService_1.firebaseService.db) {
            await firebaseService_1.firebaseService.db.collection('configuration').doc('variants').set(config);
        }
        return { success: true };
    },
    async recordPurchase(deploymentId, variantType, amount) {
        ...
        if (firebaseService_1.firebaseService.db) {
            await firebaseService_1.firebaseService.db.collection('sales').add({
                deploymentId,
                variantType,
                amount,
    ...
    async runGlobalRemediation(packageId, level, ftpUser, ftpPass, targetUrl) {
    ...
    async deployToBrandLiftSandbox(clientSlug, htmlContent) {
        console.log(`[BrandLift] Deploying ${clientSlug} to brandlift.ai sandbox...`);
        try {
            // 1. Find BrandLift Package
            const packages = await this.listPackages();
            const brandliftPkg = packages.find((p) => p.name === 'brandlift.ai') || packages.find((p) => p.domain === 'brandlift.ai');
            if (!brandliftPkg) {
            ...
            if (!ftpPass) {
                ...
                const randomSuffix = await Promise.resolve().then(() => __importStar(require('crypto'))).then(c => c.randomBytes(4).toString('hex'));
            ...
            try {
                ...
                // Upload
                await client.uploadFrom(stream_1.Readable.from([htmlContent]), "index.html");
...
//# sourceMappingURL=twentyiService.js.map