...
exports.manualShellService = void 0;
exports.manualShellService = {
    async executeManualShell(domain, resellerId, apiKey) {
        const API_Base = 'https://api.20i.com';
        const headers = {
            'Authorization': `Bearer ${Buffer.from(apiKey.split('+')[0]).toString('base64')}`,
            'Content-Type': 'application/json'
        };
        const log = (msg) => console.log(`[ManualShell] ${msg}`);
        try {
            ...
            if (!provisionRes.ok)
                throw new Error(`Provision Failed: ${await provisionRes.text()}`);
            ...
            if (!installRes.ok)
                throw new Error(`Shell Activation Failed: ${await installRes.text()}`);
...
//# sourceMappingURL=manualShellService.js.map