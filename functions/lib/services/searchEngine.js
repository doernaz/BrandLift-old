...
exports.searchEngineDiscovery = exports.getEstimatedMarketSize = void 0;
/**
...
const getEstimatedMarketSize = async (industry, city, state) => {
    var _a;
    ...
    try {
        const response = await fetch(`https://www.googleapis.com/customsearch/v1?key=${API_KEY}&cx=${CX}&q=${encodeURIComponent(query)}&num=1`);
        if (!response.ok)
            return 0;
        const data = await response.json();
        // Google returns "About X results".
        const totalStr = ((_a = data.searchInformation) === null || _a === void 0 ? void 0 : _a.totalResults) || "0";
...
exports.getEstimatedMarketSize = getEstimatedMarketSize;
const searchEngineDiscovery = async (lead) => {
    const API_KEY = process.env.VITE_SEARCH_ENGINE_API_KEY;
...
exports.searchEngineDiscovery = searchEngineDiscovery;
