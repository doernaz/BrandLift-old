...
export const firebaseService = {
    get db() {
        return initializeApp ? initializeFirebase() : null;
    },
     ...
     */
    async saveBlueprint(blueprint: any) {
        const database = initializeFirebase();
        if (!database) throw new Error("Firebase not initialized.");

        const id = blueprint.id || `bp_${Date.now()}`;
        await database.collection(COLLECTIONS.BLUEPRINTS).doc(id).set({
            ...blueprint,
     ...
     */
    async syncClientData(clientId: string, data: any) {
        const database = initializeFirebase();
        if (!database) throw new Error("Firebase not initialized. Cannot sync data.");

        await database.collection(COLLECTIONS.CLIENTS).doc(clientId).set({
            ...data,
            lastUpdated: new Date()
        }, { merge: true });
    },
     ...
     */
    async getPatentedLogic(logicId: string) {
        const database = initializeFirebase();
        if (!database) {
     ...
     */
    async getClientByPackageId(packageId: string) {
        const database = initializeFirebase();
        if (!database) throw new Error("Firebase not initialized.");
    ...
    async seedDatabase() {
        const database = initializeFirebase();
        if (!database) throw new Error("Firebase not initialized. Cannot seed database.");
