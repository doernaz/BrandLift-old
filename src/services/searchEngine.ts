...
export const getEstimatedMarketSize = async (industry: string, city: string, state: string): Promise<number> => {
    const API_KEY = import.meta.env.VITE_SEARCH_ENGINE_API_KEY;
    const CX = import.meta.env.VITE_SEARCH_ENGINE_ID; // Must be a CX that searches the whole web or at least yelp.com
