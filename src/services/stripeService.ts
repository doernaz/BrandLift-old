...
export const stripeService = {
     ...
     */
    async createCheckoutSession(sandboxId: string, email: string, tier: 'BASIC' | 'PRO' | 'ENTERPRISE' = 'BASIC', addons: { price: string, quantity: number }[] = []) {
    ...
    verifyWebhookSignature(payload: string | Buffer, signature: string) {
        if (!process.env.STRIPE_WEBHOOK_SECRET || !stripeClient) return null;
        try {
     ...
     */
    async updateTierPrice(tier: 'BASIC' | 'PRO' | 'ENTERPRISE', amountCents: number) {
        if (!stripeClient) {
            console.log(`[MOCK] Updated ${tier} price to $${amountCents / 100}`);
            PRICING_TIERS[tier].amount = amountCents;
            return { success: true, tier: PRICING_TIERS[tier] };
        }
        ...
        if (!productId) throw new Error(`Product ID for ${tier} not found in env`);
    ...
    async createPrice(productId: string, amountCents: number) {
