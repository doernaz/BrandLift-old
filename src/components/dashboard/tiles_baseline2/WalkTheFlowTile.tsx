                ...
                {activeTab === 'variants' && (
                    <div className="animate-fadeIn grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {flowState.variants.map((variant, idx) => {
                            const type = typeof variant === 'string' ? 'authority' : variant.type;
                            const colors = {
                                authority: 'bg-slate-800',
                                speedster: 'bg-indigo-900',
                                closer: 'bg-emerald-900'
                            };
                            const accent = {
                                authority: 'bg-slate-600',
                                speedster: 'bg-indigo-500',
                                closer: 'bg-emerald-500'
                            };

                            return (
                                <div key={idx} className="bg-slate-950 border border-slate-800 rounded overflow-hidden hover:border-cyan-500 transition-all group relative">
                                    <div className="aspect-video bg-slate-900 relative p-4 flex flex-col gap-2 overflow-hidden">
                                        {/* Mock Request Header */}
                                        <div className="h-2 w-full bg-slate-800 rounded-full mb-2 flex items-center px-2 gap-1">
                                        ...
                                        {/* Website Skeleton UI */}
                                        <div className={`w-full h-24 ${colors[type as keyof typeof colors] || 'bg-slate-800'} rounded-md opacity-50 flex items-center justify-center mb-1`}>
                                            <div className={`w-12 h-12 rounded-full ${accent[type as keyof typeof accent] || 'bg-slate-600'} opacity-20`}></div>
                                        </div>
                                        ...
                                        {/* Overlay Actions */}
                                        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-4 flex justify-between items-end opacity-0 group-hover:opacity-100 transition-opacity">
                                            <a
                                                href={typeof variant === 'string' ? variant : variant.url}
                                                target="_blank"
                                                rel="noreferrer"
                                                className="bg-cyan-500 text-black text-[10px] font-bold px-3 py-1 rounded flex items-center gap-1 hover:bg-cyan-400 w-full justify-center shadow-[0_0_15px_rgba(6,182,212,0.5)]"
                                            >
                                    ...
                                    <div className="p-3 border-t border-slate-800 bg-slate-950/50">
                                        <div className="flex justify-between items-start">
                                            ...
                                            {/* Blueprint Icon */}
                                            <div className={`w-6 h-6 rounded flex items-center justify-center ${colors[type as keyof typeof colors] || 'bg-slate-800'} text-white`}>
